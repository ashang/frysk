...
{
    public DebugPubs (ByteBuffer section)
    {
	super (section);
    }
    ...
    private final void parseSet (Builder builder)
    {
	...
	long debugInfoLength = getSectionLength (format);

	if (builder.buildSetHeader (format, unitLength, version,
				    debugInfoOffset, debugInfoLength)
	    == Builder.Action.PARSE)
	    parseTuples (builder, format, setLimit);
	position (setLimit);
	builder.buildSetFooter ();
    }

    private final void parseTuples (Builder builder, Format format,
				    long setLimit)
    {
	while (position () < setLimit) {
	    long compilationUnitOffset = getSectionOffset (format);
	    if (compilationUnitOffset == 0)
		break;
	    else {
		long namePosition = position ();
		skipToZeroUBYTE ();
		builder.buildSetEntry (compilationUnitOffset, namePosition);
	    }
	}
    }
}
