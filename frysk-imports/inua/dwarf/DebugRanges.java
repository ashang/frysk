...
{
    public DebugRanges (ByteBuffer section)
    {
	super (section);
    }
    ...
    {
	void buildRangeListBase (long base)
	{
	}
	void buildRangeListEntry (long begin, long end)
	{
	}
	void buildRangeListEnd ()
	{
	}
    }
	...
	while (hasRemaining ()) {
	    ...
	    if (begin == -1) {
		builder.buildRangeListBase (end);
	    }
	    else if (begin == 0 && end == 0) {
		builder.buildRangeListEnd ();
		break;
	    }
	    else
		builder.buildRangeListEntry (begin, end);
	}
    }
}
