...
class SymSection
{
    ...
    Shdr symtab;
    long nsyms;
    public SymSection (Elf elf, Shdr symtab)
	throws java.lang.Exception
    {
	this.elf = elf;
	this.symtab = symtab;
	buffer = symtab.elfBuffer ();
	nsyms = symtab.size / symtab.entsize;
    }

    public static abstract class Builder
    {
	abstract void buildSym (long position,
				long name,
				long value,
				long size,
				long info,
    ...
    private final void parseSym (Builder builder)
    {
	long position = buffer.position ();
	long name = buffer.getSignedWord ();
	long value = buffer.getSignedWord ();
	long size = buffer.getSignedWord ();
	long info = buffer.getSignedByte ();
	long other = buffer.getSignedByte ();
	int shndx = buffer.getUnsignedHalf ();
	builder.buildSym (position, name, value, size,
			  info, other, shndx);
    }
}
