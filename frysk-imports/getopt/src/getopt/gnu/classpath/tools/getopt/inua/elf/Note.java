...
{
    public long type;
    public ElfBuffer name;
    public ElfBuffer desc;
    ...
    private ElfBuffer sliceSz (ElfBuffer b, long sz)
    {
	ElfBuffer slice = b.slice (b.position (), sz);
	b.position ((b.position () + sz + 3) & -4);
	return slice;
    }

    Note (ElfBuffer b)
    {
	long namesz = b.getUnsignedWord ();
	long descsz = b.getUnsignedWord ();
	type = b.getUnsignedWord ();
	name = sliceSz (b, namesz);
	desc = sliceSz (b, descsz);
    }
