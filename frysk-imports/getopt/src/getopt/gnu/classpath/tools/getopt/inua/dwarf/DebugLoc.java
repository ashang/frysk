...
{
    public DebugLoc (ByteBuffer locSection)
    {
	super (locSection);
    }
    ...
    {
	void buildLocationListBase (long base)
	{
	}
	void buildLocationListEntry (long offset, long begin, long end,
				     DebugSection positioned, long length)
	{
	}
    ...
    public void construct (Builder builder, Format format, long offset)
    {
	position (offset);
	while (hasRemaining ()) {
	    long begin = getUWORD (format);
	    long end = getUWORD (format);
	    if (begin == -1)
		builder.buildLocationListBase (end);
	    else if (begin == 0 && end == 0) {
		builder.buildLocationListEnd ();
		break;
	    }
	    else {
		int length = getUHALF ();
		long position = position ();
		builder.buildLocationListEntry (offset, begin, end,
						this, length);
