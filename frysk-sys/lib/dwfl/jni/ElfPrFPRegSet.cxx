...
jlong
lib::dwfl::ElfPrFPRegSet::getEntrySize(jnixx::env env) {
  return getFPRegisterBuffer(env).GetArrayLength(env);
}
...
jlong 
lib::dwfl::ElfPrFPRegSet::fillMemRegion(jnixx::env env,
					jnixx::jbyteArray jbuffer,
					jlong startAddress) {
  jbyteArrayElements buffer = jbyteArrayElements(env, jbuffer);
  jbyte *bs = buffer.elements();
  jnixx::jbyteArray jfpBuf = getFPRegisterBuffer(env);
  jbyteArrayElements fpBuf = jbyteArrayElements(env, jfpBuf);
  jbyte *buff = fpBuf.elements();
  int length = GetRaw_registers(env).GetArrayLength(env);
  memcpy(bs + startAddress, buff, length);
  return length;
}
