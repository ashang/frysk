 ...
 */
public class ObserverDataModel {
	...
	public void populateData() {
		...
		ObservableLinkedList observerList = ObserverManager.theManager.getTaskObservers();
		ListIterator iter = observerList.listIterator();
		TreeIter treeIter;
		
		while (iter.hasNext())
		{
			ObserverRoot observer = (ObserverRoot) iter.next();
			treeIter = this.listStore.appendRow();
			setRow(treeIter,observer.getName());
		}
		
	}
	
	public String nameFromPath(TreePath path) {
		
		return listStore.getValue(listStore.getIter(path),this.nameDC);
