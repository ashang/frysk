    ...
    implements Saveable
{
  ...
  public DisassemblyFormatDialog (LibGlade glade)
  {
    ...
    for (i = 0; i < DisassemblyWindow.colNames.length - 1; i++)
      {

        TreeIter iter = model.appendRow();
        String text = DisassemblyWindow.colNames[i];
    ...
    TreeIter iter = model.appendRow();
    model.setValue(iter, (DataColumnBoolean) cols[0], false);
    model.setValue(iter, (DataColumnString) cols[1], DisassemblyWindow.colNames[i]);
    ...
    ((Button) this.glade.getWidget("formatCloseButton")).addListener(new ButtonListener()
    {
      public void buttonEvent (ButtonEvent arg0)
      {
        if (arg0.isOfType(ButtonEvent.Type.CLICK))
          {
            DisassemblyFormatDialog.this.save(DisassemblyFormatDialog.this.prefs);
            DisassemblyFormatDialog.this.hideAll();
          }
      }
    });
    ...
    for (int i = 0; i < DisassemblyWindow.colNames.length; i++)
      {
        ...
        prefs.putBoolean(DisassemblyWindow.colNames[i], val);
    ...
    for (int i = 0; i < DisassemblyWindow.colNames.length; i++)
      {
        boolean val = prefs.getBoolean(
                     DisassemblyWindow.colNames[i], i == 0);
