    ...
    extends TestLib
{
	...
	// what array
	e.send ("what arr_2\n");
	e.expect ("what.*int.*5,6.*funit-array.c.*" + prompt);
	// arr_2
	e.send ("print arr_2\n");
	...
	e.expect ("what.*static_class_t.*funit-struct.c.*" + prompt);
	e.send ("what class_4\n");
	...
	// class
	e.send ("what mb\n");
	e.expect ("what.*mb.*Type.*" + prompt);
	e.send("what Type\n");
	e.expect ("what.*Type.*note.*Type.*~Type.*" + prompt);
	e.send("print mb\n");
	e.expect ("print.*Base1.*Base2.*note.*\"mb\".*" + prompt);
	e.send ("what xyz\n");
	e.expect ("Derived.*" + prompt);
	e.send ("what Derived\n");
	e.expect ("what.*Base3.*Derived.*do_this.*char.*do_this_impl.*char.*" + prompt);
	e.send("print xyz\n");
	e.expect ("print.*Base3.*Base3.*" + prompt);
	e.send("quit");
	e.close();
	child.close();
    }
