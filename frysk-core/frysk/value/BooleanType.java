...
{    
    private BooleanType(String name, ByteOrder order, int size,
			IntegerType accessor) {
	super(name, order, size, accessor);
    }
    public BooleanType(String name, ByteOrder order, int size) {
	this(name, order, size,
	     (IntegerType)new UnsignedType(name, order, size));
    }
    BigInteger getBigInteger(Location location) {
	if (super.getBigInteger(location).equals(BigInteger.ZERO))
	    return BigInteger.ZERO;
	else
	    return BigInteger.ONE;
    }
    void putBigInteger(Location location, BigInteger value) {
	if (value.equals(BigInteger.ZERO))
	    super.putBigInteger(location, BigInteger.ZERO);
	else
	...
	return new BooleanType(getName(), order(), getSize(), accessor);
    }
}
