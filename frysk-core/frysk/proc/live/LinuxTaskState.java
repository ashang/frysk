    ...
    extends TaskState
{
	...
	public TaskState handleTrappedEvent (Task task)
	{
	  ...
	  if (isa.isTaskStepped(task) || task.just_started)
	      ...
	      else
		{
		  ...
		  // And we also didn't send a step request.
  ...
  // Task is stepping (either because of instruction observers
      ...
      else
	{
	  ...
	  if (blockers >= 0)
		...
		return sendContinue(task, 0);
	      ...
	      // Deliver the real Trap event to the Task.
	      // This is somewhat weird, we are either stepping a
	      // trapping instruction (breakpoint) that we didn't
	      // install, or something is sending this process
	      // an explicit trap signal.
	      return handleSignaledEvent(task, Sig.TRAP_);
	    }
	}
    }
